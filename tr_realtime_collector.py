...
def get_kap_disclosures(symbol):
   """KAP'tan son 2 günün bildirimlerini çek ve kategorize et"""
   try:
...
def calculate_signal_score(price_change, volume_ratio, kap):
   """
   US sistemiyle aynı mantık:
   - Kataliz var mı? (KAP)
   - Kurumsal para giriyor mu? (hacim)
   - Fiyat hareketi ne kadar güçlü?
   """
   score = 0
   reasons = []
   ...
   # ZORUNLU: KAP yoksa çok güçlü olmalı
   if not kap:
...
def scan_once(symbols, avg_volumes):
   """
   3 adımlı kartal gözü tarama:
   1. Hızlı fiyat+hacim tarama — güçlü adayları bul
   2. Adaylara KAP kontrol et
   3. Puanla, en iyi 5'i seç ve kaydet
   """
   candidates = []
   now = time.time()
   ...
   for symbol in symbols:
       try:
           ...
           # Ön filtre: KAP kontrol etmeye değer mi?
           # Hacim 1.5x+ VEYA fiyat 2%+ ise aday
   ...
   for c in candidates:
       try:
           ...
           conviction, reasons, score = calculate_signal_score(price_change, volume_ratio, kap)
